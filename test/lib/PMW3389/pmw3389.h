...
struct PMW3389_DATA {
  ...
  // all 1296 raw data in the current frame; * Avg value =
  // Raw_Data_Sum * 1024 / 1296
  byte maxRawData;      // Max raw data value in current frame, max=127
  byte minRawData;      // Min raw data value in current frame, max=127
  unsigned int shutter; // unit: clock cycles of the internal oscillator.
  // shutter is adjusted to keep the average raw data
  // values within normal operating ranges.
};

class PMW3389 {
  public:
    PMW3389(SPIClass *spi, int ChipSelectPin); // set CPI to 800 by default.
